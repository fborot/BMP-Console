...
namespace BMP_Console {
    public partial class EditMemberDetails : Form {
                                ...
                                if (SaveMemberInDB(temp_member)) {
                                    ...
                                    if (NumberMembers > 0) {
                                        ...
                                        for (int n = 0; n < DependentsContainer.Count; n++) {
                                            seed_member = CreateMemberBasedOnDependent((Dependent)DependentsContainer[n], seed_member, temp_member.parent_bmp_id);
        ...
        public string GetCustomerPayProfileDetails(String ApiLoginID, String ApiTransactionKey, string customerProfileId, string customerPaymentProfileId, out string cc_info) {
            string CCExpDate = string.Empty;
            cc_info = string.Empty;
        ...
        member CreateMemberBasedOnDependent(Dependent dep, member m, string p_bmp_id) {
            ...
            member new_member = new member(new_id, new_bmp_id, dep.name, dep.mi, dep.lname, m.email, m.language, m.marital_status, dep.gender, new_dob, m.home_phone_number, m.mobile_phone_number, m.other_phone_number,
                ...
                m.use_home_as_shipping_address, m.plan_name, m.plan_type, m.recurring_total, m.start_date, m.end_date, 0, m.agencyID, m.branchID, m.recurrency, m.cc_info, m.cc_type, m.cc_expiration_date, m.cc_auto_pay, m.dateadded, "No", dep.relationship, m.active, p_bmp_id, m.payment_status);
            return new_member;
        }
